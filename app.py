...
# 5. [로직 중요] 잠금 상태를 먼저 계산하기 위해 세션 상태 확인
# 시나리오 선택값이 '시나리오를 선택하세요'가 아니면 즉시 잠금
if "scenario_select" not in st.session_state:
    st.session_state.scenario_select = "시나리오를 선택하세요"

is_active = (st.session_state.scenario_select != "시나리오를 선택하세요")
is_finished = len(st.session_state.checked_scenarios) >= 4
disable_input = is_active or is_finished

# 6. 사이드바 구성 (입력창들을 먼저 배치)
...
scenario = st.sidebar.selectbox(
    ...
    key="scenario_select"  # 이 key가 세션 상태와 연동됩니다.
)
...
if scenario != "시나리오를 선택하세요":
    ...
    fig.add_trace(go.Scatter(x=t_axis, y=np.percentile(sim_results, 50, axis=1), name="중앙값(평균적 경로)"))
    fig.add_trace(go.Scatter(x=t_axis, y=np.percentile(sim_results, 10, axis=1), name="하위 10%(비관적 경로)", line=dict(dash='dot')))
    fig.add_hline(y=target_amount, line_dash="dash", line_color="red", annotation_text="목표금액선")
    fig.update_layout(xaxis_title="기간 (년)", yaxis_title="자산액 (만원)")
    st.plotly_chart(fig, use_container_width=True)

else:
    st.info("👈 왼쪽 사이드바에서 모든 조건을 입력한 후, 맨 아래 **'시나리오'**를 선택하여 검증을 시작하세요.")
...
if current_count < 4:
    st.warning(f"💡 현재 {current_count}/4 시나리오를 확인했습니다. 모든 시나리오를 확인해야 제출 가능합니다. **시나리오 선택 시 조건 변경 불가.**")
else:
    ...
    if bc1.button("🔄 시뮬레이션 조건 다시 입력", use_container_width=True):
        st.session_state.checked_scenarios = set()
        st.session_state.results_log = {}
        st.session_state.scenario_select = "시나리오를 선택하세요" # 선택창 초기화
        st.rerun()
    ...
    if bc2.button("📤 결과 제출하기", use_container_width=True):
        if not user_name or not user_id:
            ...
            try:
                ...
                st.error(f"제출 오류: {e}. 구글 시트의 헤더명이 코드와 일치하는지 확인해 주세요.")
